...
- **A8 — the access rosters.** Which surfaces the tooling can actually reach, in which access mode — TWO rosters: the finder's knowledge sources (`corpus/finder.md`) and the messages bus's traffic channels, mail · chat · calendar (`corpus/messages.md`).
...
2. **Skills layer:** materialize each adopted portable-skills entry in the tooling's skill/instructions format, homed with the project it serves (estate-wide ones in the hub); with no tooling, keep them as printed checklists in the hub. The message/meeting flow (`corpus/messages.md`) and the writer's craft lane (`corpus/communication-craft.md`) materialize here — procedures and engine jobs, the craft lane's state under the hub's engine: there is NO separate communication project (the person container is people; meeting outcomes live in the worklog).
3. **Found the infrastructure trio:** the store (`corpus/store.md`) — seed the schema home and register the engine A7 chose (markdown-with-shape bootstrap if none); the finder (`corpus/finder.md`) — seed the knowledge-source roster from A8; the messages bus (`corpus/messages.md`) — seed the routing rules and the staged-drafts registry, traffic-channel adapters from A8. All three are registry rows of kind infrastructure; every later project consumes them.
4. **Found the people project:** seed the register `people.md` per `corpus/people.md`.
5. **Found the worklog project:** seed `worklog.md` + `timelog.md` per `corpus/work-record.md` and the reports dir per `corpus/work-report.md` (expected core-adopt at every policy outcome; the seed-refresh line in `corpus/boundary-protocol.md` is the one piece depending on A2). The PUBLIC view's home is an intake question (`knowledge/week1-intake.md` §15) — until it resolves, seed the staging file `worklog-public.md`; once a team surface is named, render there and retire the staging file.
6. **Found the benefits project:** seed the register `benefits.md` per `corpus/benefits.md`; its first standing action is catalog discovery on the employer's benefits/program portals (intake §19) — at a fresh employment, the new-hire enrollment window is read off the portal FIRST (short window, usually irreversible once closed).
7. **Found the career project:** seed `review.md` + `promotion.md` per `corpus/career.md`, shaped by the review-instrument and leveling answers intake §20 has already surfaced, plus `growth.md` (development investments) and `themes.md` (positioning); register the company-watch capture job in the engine — its signals type into the store, person-signals feed the people cards, themes and the reward map feed `themes.md`.
...
9. **Found the risk project:** seed `risks.md` per `corpus/risk-register.md`.
10. **Found the research project** (only where intake §18 cleared any personal-stack presence): seed the satellite registry `research.md` per `corpus/research-infrastructure.md`; clones and credentials follow that module's setup order, each landing a footprint-register line.
11. **Found the main-work project — the manager of all product work:** seed the portfolio board `projects.md` per `corpus/project-lifecycle.md`, pre-filling candidates from the intake's project-landscape questions (`knowledge/week1-intake.md` §12). Per-project homes are founded UNDER main-work on demand at accept, via the project-founding procedure (`corpus/portable-skills.md`; each registered in the estate registry, dev record per `corpus/dev-workflow.md`) — never pre-seeded.
