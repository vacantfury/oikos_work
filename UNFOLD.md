...
- **A8 — the channel roster.** Which of the job's channels (mail, chat, calendar, docs/wikis, code/reviews, tickets, dashboards, people directory, sanctioned public search) the tooling can actually reach, each with its access mode (`corpus/search.md`).
...
2. **Skills layer:** materialize each adopted portable-skills entry in the tooling's skill/instructions format, homed with the project it serves (estate-wide ones in the hub); with no tooling, keep them as printed checklists in the hub. The message/meeting flow (`corpus/communication-flow.md`) and the writer's craft lane (`corpus/communication-craft.md`) materialize here — procedures and engine jobs, the craft lane's state under the hub's engine: there is NO separate communication project (the person container is people; meeting outcomes live in the worklog).
3. **Found the infrastructure pair:** the store (`corpus/store.md`) — seed the schema home and register the engine A7 chose (markdown-with-shape bootstrap if none) — and the search bus (`corpus/search.md`) — seed the channel-roster table from A8. Both are registry rows of kind infrastructure; every later project consumes them.
4. **Found the people project:** seed the register `people.md` per `corpus/connections.md`.
5. **Found the company project:** seed `company.md` per the signal-log + four-register format in `corpus/company-watch.md`, pre-filling the leadership chain, direction surfaces, and initial themes the intake's company-baseline questions (`knowledge/week1-intake.md` §13) have already surfaced.
...
11. **Found the research project** (only where intake §18 cleared any personal-stack presence): seed the satellite registry `research.md` per `corpus/research-infrastructure.md`; clones and credentials follow that module's setup order, each landing a footprint-register line.
